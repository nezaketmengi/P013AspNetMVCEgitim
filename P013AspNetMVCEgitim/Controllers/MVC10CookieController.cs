    ...
    public class MVC10CookieController : Controller
    {
        ...
        public IActionResult CookieOlustur(string kullaniciAdi, string sifre)
        {
            ...
            if (kullaniciAdi == "admin" && sifre == "123")
            {
                ...
                {
                    Expires = DateTime.Now.AddMinutes(1) //bu ayarı kullanarak oluşturacağımız cookie ye 1 dk lık yaşam süresi belirledik.
                };
                Response.Cookies.Append("kullaniciAdi", kullaniciAdi, cookieAyarlari);
                Response.Cookies.Append("sifre", sifre, cookieAyarlari);
                Response.Cookies.Append("userquid", Guid.NewGuid().ToString()); // Guid.NewGuid() metoduyla kullanıcıya özel benzersiz bir numara oluşturduk.
